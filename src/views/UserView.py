...
def create():
    ...
    user = UserModel(data)
    token = Auth.generate_confirmation_email_token(user.email)
...
def confirm(token):
    """
        Confirm token
    """
    app.logger.info('llega siquiera al confirm token--------------#'+token)
    try:
        email = Auth.confirm_email_token(token)
    except:
        return custom_response('The confirmation link is invalid or has expired.', 403)
    user = UserModel.get_user_by_email(email)
    if user.confirmed:
        message = {'error': 'Account already confirmed. Please login.'}
        return custom_response(message, 400)
    else:
        user.confirmed = True
        user.email_token= ''
        user.save()
    #agregar los envios de email
    return custom_response({'success': 'You have confirmed your account. Thanks!'}, 200)
...
def login():
    ...
    if not user.confirmed:
        return custom_response({'error': 'user not confirmed'}, 403)
...
def loginfg():
    ...
    user = UserModel.get_user_by_email(data.get('email'))
    if not user:
        return custom_response({'error': 'email does not exist'}, 403)
    if not user.confirmed:
        return custom_response({'error': 'user not confirmed'}, 403)
    # if not user.check_hash(data.get('password')):
